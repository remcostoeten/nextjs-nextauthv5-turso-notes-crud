...
export function NewsButton() {
  return (
    <Link
      href="/news"
      className={`inline-flex gap-x-6 bg-white/5 items-center ${UI_CONFIG.BORDERS.ROUNDED} p-1 pr-6 border ${UI_CONFIG.FONTS.SIZES.SMALL} font-medium duration-150 ${UI_CONFIG.ANIMATIONS.HOVER}`}
    >
      <span
        className={`inline-block ${UI_CONFIG.BORDERS.ROUNDED} px-3 py-1 bg-${UI_CONFIG.COLORS.ACCENT} text-${UI_CONFIG.COLORS.PRIMARY}`}
      >
        News
      </span>
