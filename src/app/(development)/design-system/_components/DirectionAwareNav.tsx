...
import { TooltipTrigger } from '@/components/ui'
...
const Tab = ({ href, label, isActive, disabled }: TabProps) => {
    ...
    const tabContent = (
      ...
      ${isActive ? 'bg-dark-section--lighter text-subtitle' : 'bg-transparent text-subtitle hover:text-title'} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
...
export default function DesignSystemTabs() {
    ...
    return (
        <nav className="w-full pt-[12px]  border-top-seperator z-50 fixed border-t border-seperator  bottom-0 bg-section ">
            <div className="max-w-full mx-auto px-4 relative">
                {showLeftArrow && (
                    <button
                        onClick={() => scroll('left')}
                        className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-section-lighter rounded-full p-1 z-10"
                    >
                        <ChevronLeft size={24} />
                    </button>
                )}
